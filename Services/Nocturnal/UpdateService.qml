...
Singleton {
  ...
  readonly property string upgradeLogBaseUrl: Quickshell.env("NOCTURNAL_UPGRADELOG_URL") || "https://api.nocturnal.dev/upgradelog"
      ...
      if (request.readyState === XMLHttpRequest.DONE) {
        ...
        if (request.status >= 200 && request.status < 300) {
          ...
          if (request.status === 404) {
            ...
            fetchError = "Unable to load changelog data. Please try again later.";
            openWhenReady();
          }
        }
      }

...
Singleton {
  ...
  // Plugin updates that require a newer Nocturnal version: { pluginId: { currentVersion, availableVersion, minNocturnalVersion } }
  ...
  function installPlugin(pluginMetadata, skipCollisionCheck, callback) {
    ...
    if (!skipCollisionCheck) {
      ...
      // Check Nocturnal version compatibility (skip when updating - that's handled in performUpdateCheck)
      if (pluginMetadata.minNocturnalVersion) {
        var nocturnalVersion = UpdateService.baseVersion;
        if (compareVersions(pluginMetadata.minNocturnalVersion, nocturnalVersion) > 0) {
          var incompatibleMsg = "{plugin} requires Nocturnal v{version} or higher";
  ...
  function loadPluginData(pluginId, manifest, callback) {
    ...
    loadPluginSettings(pluginId, function (settings) {
      // Then load translations (English-only)
  ...
  function createPluginAPI(pluginId, manifest, settings, translations, fallbackTranslations) {
    ...
    // Set current language (English-only)
  ...
  function performUpdateCheck() {
    ...
    for (var i = 0; i < installedIds.length; i++) {
      ...
      if (installedManifest && availablePlugin) {
        ...
        if (compareVersions(availableVersion, currentVersion) > 0) {
          // Check if the available version requires a higher Nocturnal version
          if (availablePlugin.minNocturnalVersion) {
            var nocturnalVersion = UpdateService.baseVersion;
            if (compareVersions(availablePlugin.minNocturnalVersion, nocturnalVersion) > 0) {
              Logger.d("PluginService", "Pending update for", pluginId + ": requires Nocturnal v" + availablePlugin.minNocturnalVersion + " (current: v" + nocturnalVersion + ")");
              pendingUpdates[pluginId] = {
                currentVersion: currentVersion,
                availableVersion: availableVersion,
                minNocturnalVersion: availablePlugin.minNocturnalVersion
              };
              continue;
            }
          }
    ...
    if (updateCount > 0) {
      ...
      Logger.i("PluginService", pendingCount, "plugin update(s) pending (require newer Nocturnal)");
    } else {
  ...
  function updatePlugin(pluginId, callback) {
    ...
    // Check Nocturnal compatibility
    if (availablePlugin.minNocturnalVersion) {
      // Simple check: just warn, don't block (UpdateService would have more sophisticated logic)
      Logger.d("PluginService", "Plugin requires Nocturnal v" + availablePlugin.minNocturnalVersion);
    }
  ...
  function setupPluginFileWatcher(pluginId) {
    ...
    // Watch the English translation file for this plugin (hot-reload support)
  ...
  function updateTranslationWatchers() {
    for (var pluginId in root.pluginFileWatchers) {
      ...
      // Create new watcher for English (hot-reload)

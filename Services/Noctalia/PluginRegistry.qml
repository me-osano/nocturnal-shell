...
Singleton {
  ...
  readonly property string mainSourceUrl: "https://github.com/me-osano/nocturnal-plugins"
  ...
  // Check if a source is the main Nocturnal plugins repository
  ...
  function migratePluginData() {
    ...
    // Migration: rename "Official Nocturnal Plugins" -> "Nocturnal Plugins"
    var newSources = [];
    var sourcesChanged = false;
    for (var i = 0; i < root.pluginSources.length; i++) {
      var source = root.pluginSources[i];
      if (source.name === "Official Nocturnal Plugins") {
        newSources.push({
                          name: "Nocturnal Plugins",
                          url: source.url,
        ...
        sourcesChanged = true;
        Logger.i("PluginRegistry", "Migrating source name: 'Official Nocturnal Plugins' -> 'Nocturnal Plugins'");
      } else {
